...
def main(cif_dir, gt_prefix="gt", pred_prefix="pred", num_cpus=48):
    ...
    get_idx = lambda f:int(f.stem.split('_')[-1])
    paired_files = [(d / f'{gt_prefix}_{get_idx(d)}.cif', d / f'{pred_prefix}_{get_idx(d)}.cif' if (d / f'{pred_prefix}_{get_idx(d)}.cif').exists() else None) for d in sample_dirs]
