...
bb_emb_mean = [ 1.9883,  3.4131, -1.2937, -1.0239,  1.1878, -1.1955,  2.8939, -0.6505,
...
bb_emb_std = [2.9326, 8.1418, 7.1256, 3.7709, 3.2484, 2.8743, 4.1241, 3.7445, 2.7989,
...
class CrysBFN(bfnBase):
    def __init__(
        ...
        **kwargs
        ...
        self.sigma1_type = tensor(sigma1_type).to(device)
    ...
    def interdependency_modeling(
        ...
        mu_type_t_in = mu_type_t

        lattice_final, coord_final, rot_final, type_final = self.net.forward(
            time_embed, mu_type_t_in, mu_pos_t_in, mu_rot_t_in, mu_lattices_t_in, num_atoms, segment_ids, log_acc, log_acc_sphere
        )
        # out for coord
        ...
        return coord_pred, lattice_pred, rot_pred, type_pred
    ...
    def loss_one_step(self, 
        ...
        # normalize bb
        bb_embs = self.normalized_bb_emb(bb_embs)
        ...
        mu_type_t, gamma_type = self.continuous_var_bayesian_flow(
                                            x=bb_embs, t=t_per_atom, 
                                            sigma1=self.sigma1_type)
        

        # print(rot_vecs.shape, t_index_per_atom.shape)
        ...
        coord_pred, lattice_pred, rot_pred, type_pred = self.interdependency_modeling(
            t_index = t_index,
            mu_pos_t=mu_pos_t,
            mu_rot_t=mu_rot_t,
            mu_type_t=mu_type_t,
        ...
        type_loss = self.dtime4continuous_loss(
            i=t_index_per_atom,
            N=self.dtime_loss_steps,
            sigma1=self.sigma1_type,
            x_pred=type_pred,
        ...
        return lattice_loss.mean(), coord_loss.mean(), rot_loss.mean(), type_loss.mean()
    ...
    def init_params(self, num_atoms, segment_ids, batch, samp_acc_factor, start_idx, method = 'train'):
        if method == 'rand':
            ...
            mu_type_t = torch.zeros((num_batch_atoms, self.K)).to(self.device) # [N, K] continuous prior for latent space
            ...
            return num_molecules, mu_pos_t, mu_rot_t, mu_type_t, mu_lattices_t, log_acc, log_acc_sphere, num_atoms, segment_ids, rho_type, rho_lattice
        else:
            raise NotImplementedError

    def update_params(self, i, sample_steps, coord_pred, lattice_pred, rot_pred, type_pred, mu_pos_t, log_acc, mu_rot_t, log_acc_sphere, mu_type_t, rho_type, mu_lattices_t, rho_lattice, num_atoms, strategy='end_back'):
        num_molecules = lattice_pred.shape[0]
        if strategy == 'end_back':
            if i + 1 > sample_steps:
                return mu_pos_t, log_acc, mu_rot_t, log_acc_sphere, mu_type_t, rho_type, mu_lattices_t, rho_lattice
            ...
            tplus1_per_atom = tplus1_per_mol.repeat_interleave(num_atoms, dim=0).unsqueeze(-1)
            ...
            mu_type_t = self.continuous_var_bayesian_flow(
                x=type_pred,
                t=tplus1_per_atom.squeeze(-1),
                sigma1=self.sigma1_type,
                # n_samples=int(samp_acc_factor)
            )[0]
            alpha_type = torch.pow(self.sigma1_type, -2 * i / sample_steps) * (
                    1 - torch.pow(self.sigma1_type, 2 / sample_steps)
            )
            rho_type = rho_type + alpha_type
            return mu_pos_t, log_acc, mu_rot_t, log_acc_sphere, mu_type_t, rho_type, mu_lattices_t, rho_lattice
            ...
            y_type = type_pred + torch.randn_like(type_pred) * torch.sqrt(
                1 / alpha_type
            )       
            mu_type_t = (rho_type * mu_type_t + alpha_type * y_type) / (
                rho_type + alpha_type
            )
            rho_type = rho_type + alpha_type
            ...
            return  mu_pos_t, self.norm_logbeta((acc_next+self.epsilon).log()), mu_rot_t, log_acc_sphere, mu_type_t, rho_type, mu_lattices_t, rho_lattice 
        else:
            raise NotImplementedError

    def normalized_bb_emb(self, ori_emb):

        device = ori_emb.device
        z_mean = torch.FloatTensor(bb_emb_mean).to(device)
        z_std = torch.FloatTensor(bb_emb_std).to(device)
        return (ori_emb - z_mean) / z_std
    ...
    def sample(
        ...
        **kwargs
        ...
        ret_coord_pred, ret_lattice_pred, ret_rot_pred, ret_type_pred = None, None, None, None
        for sample_pass_idx in range(sample_passes):
            ...
            num_molecules, mu_pos_t, mu_rot_t, mu_type_t, mu_lattices_t, log_acc, log_acc_sphere, num_atoms, segment_ids, rho_type, rho_lattice = self.init_params(
            ...
            for i in tqdm(range(1,sample_steps+1),desc='Sampling',disable=not show_bar):
                ...
                coord_pred, lattice_pred, rot_pred, type_pred = \
                ...
                if strategy == 'end_back':
                    mu_pos_t, log_acc, mu_rot_t, log_acc_sphere, mu_type_t, rho_type, mu_lattices_t, rho_lattice = \
                        self.update_params(i, sample_steps, coord_pred, lattice_pred, rot_pred, type_pred,
                                           mu_pos_t, log_acc, mu_rot_t, log_acc_sphere, mu_type_t, rho_type, mu_lattices_t, rho_lattice, num_atoms, strategy='end_back')                    
                elif strategy == 'vanilla':
                    mu_pos_t, log_acc, mu_rot_t, log_acc_sphere, mu_type_t, rho_type, mu_lattices_t, rho_lattice = \
                        self.update_params(i, sample_steps, coord_pred, lattice_pred, rot_pred, type_pred,
                                           mu_pos_t, log_acc, mu_rot_t, log_acc_sphere, mu_type_t, rho_type, mu_lattices_t, rho_lattice, num_atoms,'vanilla')
                elif strategy == 'mix':
        ...
        ret_type_pred = type_pred if ret_type_pred is None else ret_type_pred
        if return_traj:
            return ret_coord_pred, ret_lattice_pred, ret_rot_pred, ret_type_pred, traj
        return ret_coord_pred, ret_lattice_pred, ret_rot_pred, ret_type_pred
