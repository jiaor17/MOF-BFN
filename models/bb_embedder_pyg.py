...
class BuildingBlockEmbedder(nn.Module):
    ...
    def forward(self, batch):
        ...
        atom_types = batch.atom_types        # [Nb]
        local_coords = batch.local_coords    # [Nb, 3]
        # bb_num_vec = batch['bb_num_vec'][0]     # [M]
        device = local_coords.device
        ...
        node_attr = self.atom_type_embedder(atom_types - 1)     # [Nb, D]
        ...
        bb_vector = torch.cat([torch.full((num,), i) for i, num in enumerate(bb_num_vec)]).int().to(device)
        intra_edge_index = radius_graph(local_coords, r=self._bb_cfg.max_radius, batch=bb_vector, loop=False)
        ...
        row, col = intra_edge_index
        edge_vec = local_coords[row] - local_coords[col]
        edge_dist = edge_vec.norm(dim=-1)
        intra_edge_attr = self.edge_dist_embedder(edge_dist) 
        ...
        # Pooling
        bb_attr = global_mean_pool(node_attr, bb_vector.long(), size = len(bb_num_vec))
        # assert torch.allclose(bb_attr[0], bb_attr[-1]), f"bb_attr[0]: {bb_attr[0]}, bb_attr[-1]: {bb_attr[-1]}"

        return bb_attr


class BuildingBlockEmbedderSp(nn.Module):
    ...
    def forward(self, atom_types, local_coords, bb_num_vec):
...
class BuildingBlockEmbedderCluster(nn.Module):
    ...
    def forward(self, batch):
        ...
        # Pooling
        bb_attr = global_mean_pool(node_attr, bb_vector.long(), size = len(bb_num_vec))
        clus_attr = global_mean_pool(node_attr, batch.cluster_index)
        # assert torch.allclose(bb_attr[0], bb_attr[-1]), f"bb_attr[0]: {bb_attr[0]}, bb_attr[-1]: {bb_attr[-1]}"

        return bb_attr, clus_attr


class BuildingBlockEmbedderClusterSp(nn.Module):
    ...
    def forward(self, atom_types, local_coords, bb_num_vec, cluster_index):
        ...
        # Pooling
        bb_attr = global_mean_pool(node_attr, bb_vector.long(), size = len(bb_num_vec))
        clus_attr = global_mean_pool(node_attr, cluster_index)
