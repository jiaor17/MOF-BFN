...
from models.crysbfn_csp_bhm_module import CrysBFN_CSP_PL_Model
from common.utils import PROJECT_ROOT
from experiments import utils as eu
# from common.callbacks import RecoveryCallback
from common.callbacks import EMACallback
import wandb
...
class Experiment:
    ...
    def __init__(self, *, cfg: DictConfig):
        ...
        self._module: LightningModule = CrysBFN_CSP_PL_Model(self._cfg, device=self.running_device)
    ...
    def _setup_dataset(self):
        self._train_dataset = MOFDataset(
            cache_path=os.path.join(self._data_cfg.cache_dir, 'train.lmdb'),
            dataset_cfg=self._data_cfg,
            is_training=True,
        )
        self._valid_dataset = MOFDataset(
            cache_path=os.path.join(self._data_cfg.cache_dir, 'val.lmdb'),
            dataset_cfg=self._data_cfg,
            is_training=False,
        )
    ...
    def train(self):
        ...
        if self._exp_cfg.debug:
            ...
            if hasattr(self._cfg, 'ema') and self._cfg.ema.enable:
                callbacks.append(EMACallback(decay=self._cfg.ema.decay,ema_device=self.running_device))
...
@hydra.main(version_base=None, config_path=str(PROJECT_ROOT / "configs"), config_name="bfn_base_bhm.yaml")
