...
class BaseSampleSO3(nn.Module):
    ...
    def _setup_lookup(
        ...
        if cache_dir is not None:
            ...
            cache = SO3LookupCache(cache_dir, cache_name, overwrite=False)
...
def generate_lookup_table(
    ...
    l_max: int = 1000,
    tol: float = 1e-7,
    cache_dir = '/mnt/ai4sci_develop_fast/jiaorui/projects/utils'
):
    """
    ...
    cache_file = os.path.join(cache_dir, f'{base_function.__name__}_{n_omega}_{n_sigma}.pt')

    if os.path.exists(cache_file):
        f_table = torch.load(cache_file)
        f_table = f_table.to(device=omega_grid.device, dtype=omega_grid.dtype)
...
def matrix_to_axis_angle(R, eps=1e-6):
    """
    将旋转矩阵转换为轴角形式。
    Args:
        R: 旋转矩阵，形状为 (..., 3, 3)
    ...
    Returns:
        ...
        angle: 旋转角度（弧度），形状为 (...)
    """
    # 确保输入是合法的旋转矩阵（正交且行列式为1）
    # 此处假设输入已预处理，否则需添加正交化步骤
    ...
    angle = torch.where(small_angle, 0.0, angle)

    # 初始化默认轴（例如 [1, 0, 0]）
    default_axis = torch.zeros_like(R[..., 0])
    default_axis[..., 0] = 1.0

    # 计算旋转轴
    # 反对称矩阵 R - R^T
    antisym = R - R.transpose(-2, -1)
    axis_unnormalized = torch.stack([
        antisym[..., 2, 1],  # R[2,1] - R[1,2]
        antisym[..., 0, 2],  # R[0,2] - R[2,0]
        antisym[..., 1, 0]   # R[1,0] - R[0,1]
    ], dim=-1)

    # 归一化轴
    axis_norm = torch.linalg.norm(axis_unnormalized, dim=-1, keepdim=True)
    axis = axis_unnormalized / (axis_norm + eps)

    # 处理 θ ≈ π 的情况（需特征向量法）
    near_pi = angle > (torch.pi - eps)
    if torch.any(near_pi):
        # 当 θ ≈ π 时，通过 R + I 的特征向量计算轴
        _, eigvecs = torch.linalg.eigh(R)
        # 取实特征向量（对应特征值为1）
        axis_pi = eigvecs[..., -1]  # 特征值按升序排列，最后一个对应1
        axis_pi = axis_pi.real  # 确保为实数
...
def axis_angle_to_matrix(axis, angle, eps=1e-6):
    ...
    # 处理小角度情况（返回单位矩阵）
    small_angle = torch.abs(angle) < eps
    ...
    # 构造叉乘矩阵的行
    row1 = torch.stack([zeros, -a_z, a_y], dim=-1)  # (..., 3)
    row2 = torch.stack([a_z, zeros, -a_x], dim=-1)
    row3 = torch.stack([-a_y, a_x, zeros], dim=-1)
    cross_matrix = torch.stack([row1, row2, row3], dim=-2)  # (..., 3, 3)
    
    # 罗德里格斯公式
    I = torch.eye(3, device=axis.device, dtype=axis.dtype)
    I_expanded = I.view((1,) * len(angle.shape) + (3, 3)).expand_as(outer)
    
    term1 = cos_theta[..., None, None] * I_expanded
    term2 = (1 - cos_theta[..., None, None]) * outer
    term3 = sin_theta[..., None, None] * cross_matrix
    R = term1 + term2 + term3
    
    # 强制小角度情况返回单位矩阵
    R = torch.where(small_angle[..., None, None], I_expanded, R)
